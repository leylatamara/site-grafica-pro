...
function renderizarListaClientes() {
    ...
    if (!listaEl) return;
    const filtrados = clientesCache.filter(c => (c.nome?.toLowerCase().includes(termo)) || (c.telefone?.includes(termo)) || (c.email?.toLowerCase().includes(termo)));
    listaEl.innerHTML = '';
...
function exibirDetalhesClienteEProcurarPedidos(id) {
    ...
    pedidosEl.innerHTML = pFiltrados.map(p => `...`).join(''); // Lógica de renderização de pedidos
}
...
function carregarClientes(onUpdate) {
    ...
    }, e => showNotification({ message: "Erro ao carregar clientes.", type: 'error' }));
}

async function handleCadastroCliente(e) {
    e.preventDefault();
    const form = e.target;
    const data = { nome: form.clienteNome.value, tipoCliente: form.clienteTipo.value, telefone: form.clienteTelefone.value, email: form.clienteEmail.value, cpfCnpj: form.clienteCpfCnpj.value, endereco: form.clienteEndereco.value, criadoEm: Timestamp.now() };
    if (!data.nome.trim()) { showNotification({ message: "O nome do cliente é obrigatório.", type: 'warning' }); return; }
    try { await addDoc(collection(db, `artifacts/${shopInstanceAppId}/clientes`), data); showNotification({ message: 'Cliente registado!', type: 'success' }); form.reset(); } catch (err) { showNotification({ message: 'Erro ao registar cliente.', type: 'error' }); }
}
...
async function handleSalvarEdicaoCliente(e) {
    ...
    const dados = { nome: form.clienteNomeEditar.value, tipoCliente: form.clienteTipoEditar.value, telefone: form.clienteTelefoneEditar.value, email: form.clienteEmailEditar.value, cpfCnpj: form.clienteCpfCnpjEditar.value, endereco: form.clienteEnderecoEditar.value };
    try { await updateDoc(doc(db, `artifacts/${shopInstanceAppId}/clientes`, id), dados); showNotification({ message: 'Cliente atualizado!', type: 'success' }); fecharModalEspecifico('modalEditarClienteOverlay'); } catch (err) { showNotification({ message: 'Erro ao atualizar cliente.', type: 'error' }); }
}

function excluirCliente(id, nome) {
    showNotification({ message: `Excluir o cliente ${nome}?`, type: 'confirm-delete', onConfirm: async () => {
        try { await deleteDoc(doc(db, `artifacts/${shopInstanceAppId}/clientes`, id)); showNotification({ message: 'Cliente excluído.', type: 'success' }); } catch (err) { showNotification({ message: 'Erro ao excluir.', type: 'error' }); }
    }});
}
